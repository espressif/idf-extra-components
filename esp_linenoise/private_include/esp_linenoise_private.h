 ...
 * to return.
 *
 * @param user_ctx opaque pointer used to pass the linenoise instance state
 ...
 *
 * @param state linenoise instance state containing the eventfd to set
 * @return esp_err_t ESP_OK on success, other errors on failures
 */
__attribute__((weak)) esp_err_t esp_linenoise_set_event_fd(esp_linenoise_state_t *state);
