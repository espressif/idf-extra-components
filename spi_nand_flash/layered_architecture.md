...
For FATFS filesystem support, use the separate [`spi_nand_flash_vfs`](../spi_nand_flash_vfs) component which provides:
- Diskio adapters for both legacy and BDL APIs
...
**Note:** If your project previously used FatFS APIs (e.g., `esp_vfs_fat_nand_mount()`) directly from this component, you must now add the [`spi_nand_flash_vfs`](../spi_nand_flash_vfs) component as a dependency. FatFS/VFS integration has been separated into its own component.
...
**Decoupled Filesystem**: FatFS/VFS integration handled by separate `spi_nand_flash_vfs` component
