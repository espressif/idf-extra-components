...
DRESULT ff_blockdev_read(BYTE pdrv, BYTE *buff, DWORD sector, UINT count)
{
    ...
    for (uint32_t i = sector; i < (sector + count); i++) {
        ret = bdl->ops->read(bdl, buff, sector_size, sector * sector_size, sector_size);
        if (ret != ESP_OK) {
...
DRESULT ff_blockdev_write(BYTE pdrv, const BYTE *buff, DWORD sector, UINT count)
{
    ...
    for (uint32_t i = sector; i < (sector + count); i++) {
        ret = bdl->ops->write(bdl, buff, sector * sector_size, sector_size);
        if (ret != ESP_OK) {
...
DRESULT ff_blockdev_ioctl(BYTE pdrv, BYTE cmd, void *buff)
{
    ...
    case GET_BLOCK_SIZE: {
        *((DWORD *)buff) = bdl->geometry.erase_size;
