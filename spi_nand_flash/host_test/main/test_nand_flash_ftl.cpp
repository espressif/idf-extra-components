          ...
          "[ftl][path_cache]")
{
    ...
    /* Same sector again, nothing written in between. trace_path() always
     * restarts from the journal root, so only a persistent cache can
     * make this cheaper -- the trie can't explain a drop here. */
    nand_emul_clear_stats(dev);
    REQUIRE(spi_nand_flash_read_sector(dev, rbuf, TARGET) == ESP_OK);
    size_t warm_reads = 0;
    ...
    destroy_ftl_dev(dev);
#else
    SKIP("CONFIG_NAND_ENABLE_STATS disabled; skipping read-count assertion.");
          ...
          "[ftl][path_cache]")
{
    ...
    /* Cheap again immediately -- proves the earlier miss was the
     * invalidation, not a permanently broken cache. */
    nand_emul_clear_stats(dev);
    REQUIRE(spi_nand_flash_read_sector(dev, rbuf, TARGET) == ESP_OK);
...
TEST_CASE("FTL read after an explicit GC pass returns correct, live data",
          "[ftl][path_cache]")
{
    ...
    const size_t SMALL_FLASH = (size_t)2u * 1024u * 1024u; /* 16 blocks, 64 pages/block */
    ...
    /* Comfortably more than one block's worth of pages (64), so the
     * oldest journal block dhara_map_gc() reclaims below definitely
     * holds real (now-obsolete) writes, not just an empty tail. */
    ...
    /* Deterministic GC trigger -- no need to guess if auto-GC fired.
     * Erase itself is deferred to a later write (prepare_head() in
     * journal.c), so there's no stats signal to check here. */
    REQUIRE(spi_nand_flash_gc(dev) == ESP_OK);

    /* TARGET_SECTOR's page was live in the reclaimed block (or copied
     * forward by a prior GC pass); its data and mapping must survive. */
    REQUIRE(spi_nand_flash_read_sector(dev, rbuf, TARGET_SECTOR) == ESP_OK);
