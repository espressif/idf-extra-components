...
TEST_CASE("erase nand flash using block device interface", "[spi_nand_flash]")
{
    ...
    TEST_ESP_OK(bdl_handle->erase(bdl_handle, 0, bdl_handle->geometry.disk_size));
...
static void do_single_write_test(spi_nand_flash_device_t *flash, uint32_t start_sec, uint16_t sec_count, esp_blockdev_handle_t bdl)
{
    ...
    esp_blockdev_cmd_arg_stat_t stats = {0};
    bdl->ioctl(bdl, ESP_BLOCKDEV_CMD_STATISTICS, &stats);
    uint32_t sector_num = stats.available_space / bdl->geometry.write_size;
...
static void test_write_nand_flash_sectors(spi_nand_flash_io_mode_t mode, uint8_t flags)
{
    ...
    esp_blockdev_cmd_arg_stat_t stats = {0};
    bdl_handle->ioctl(bdl_handle, ESP_BLOCKDEV_CMD_STATISTICS, &stats);
    uint32_t sector_num = stats.available_space / sector_size;
