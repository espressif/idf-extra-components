...
static esp_err_t spi_nand_flash_wl_blockdev_read(esp_blockdev_handle_t handle, uint8_t *dst_buf, size_t dst_buf_size, uint64_t src_addr, size_t data_read_len)
{
    ...
    uint32_t sector_size = handle->geometry.read_size;
    ...
    // Check alignment
    if ((src_addr % sector_size) != 0) {
        ESP_LOGE(TAG, "Source address 0x%llx not aligned to sector size %u", src_addr, sector_size);
        return ESP_ERR_INVALID_ARG;
    }

    uint32_t sector_id = (uint32_t)(src_addr >> dev_handle->chip.log2_page_size);
    esp_err_t res = spi_nand_flash_read_sector(dev_handle, dst_buf, sector_id);
    return res;
}
...
{
    spi_nand_flash_device_t *dev_handle = (spi_nand_flash_device_t *)((esp_blockdev_handle_t)handle->ctx)->ctx;
    uint32_t sector_size = handle->geometry.write_size;
    ...
    // Check alignment
    if ((dst_addr % sector_size) != 0) {
        ESP_LOGE(TAG, "Write address 0x%llx not aligned to sector size %u", dst_addr, sector_size);
        return ESP_ERR_INVALID_ARG;
    }
    uint32_t sector_id = (uint32_t)(dst_addr >> dev_handle->chip.log2_page_size);
    esp_err_t res = spi_nand_flash_write_sector(dev_handle, src_buf, sector_id);
    return res;
}
...
static esp_err_t spi_nand_flash_wl_blockdev_erase(esp_blockdev_handle_t handle, uint64_t start_addr, size_t erase_len)
{
    ...
    if ((start_addr % sector_size) != 0) {
        ESP_LOGE(TAG, "Start address 0x%llx not aligned to sector size %u", start_addr, sector_size);
        return ESP_ERR_INVALID_ARG;
    }

    uint32_t sector_count = (uint32_t)(erase_len >> dev_handle->chip.log2_page_size);
    uint32_t start_sector = (uint32_t)(start_addr >> dev_handle->chip.log2_page_size);
    for (uint32_t sector = start_sector; sector < start_sector + sector_count; sector++) {
        res = spi_nand_flash_trim(dev_handle, sector);
        if (res != ESP_OK) {
            ESP_LOGE(TAG, "%s, Failed to erase the sector", __func__);
            return res;
        }
    }
...
static esp_err_t spi_nand_flash_wl_blockdev_ioctl(esp_blockdev_handle_t handle, const uint8_t cmd, void *args)
{
    ...
    case ESP_BLOCKDEV_CMD_GET_AVAILABLE_SECTORS: {
        ...
        ret = spi_nand_flash_get_capacity(dev_handle, sectors_available);
    }
    break;

    case ESP_BLOCKDEV_CMD_TRIM_SECTOR: {
        uint32_t *sector = (uint32_t *)args;
        ret = spi_nand_flash_trim(dev_handle, *sector);
    }
...
esp_err_t spi_nand_flash_wl_get_blockdev(esp_blockdev_handle_t nand_bdl, esp_blockdev_handle_t *out_bdl_handle_ptr)
{
    ...
    spi_nand_flash_get_capacity(dev, &sectors_available);
    uint32_t sector_size = dev->chip.page_size;
    uint32_t block_size = dev->chip.block_size;

    blockdev->geometry.disk_size = sectors_available * sector_size;
