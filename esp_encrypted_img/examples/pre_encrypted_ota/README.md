...
* Create server_cert directory, Navigate to server_cert directory `cd server_cert`.
...
* Note - If you don't want to create certificates then just run the `pytest_pre_encrypted_ota.py` without passing `server_cert` directory, the server will use the hardcoded certificates present in `pytest_pre_encrypted_ota.py`
