...
static int decipher_gcm_key(const char *enc_gcm, esp_encrypted_img_t *handle)
{
    ...
    mbedtls_rsa_context *rsa_ctx = calloc(1, sizeof(mbedtls_rsa_context));
    mbedtls_rsa_init(rsa_ctx);
    mbedtls_rsa_set_padding(rsa_ctx, padding, MBEDTLS_MD_SHA256);
    ret = mbedtls_pk_setup_rsa_alt(&pk, rsa_ctx, esp_ds_rsa_decrypt, NULL, esp_ds_get_keylen);
