...
#define UNCOMPRESSED_SECP256R1_SIZE (1 + (2 * 32)) // 1 byte for uncompressed point, 32 bytes for x and 32 bytes for y
#endif /* CONFIG_PRE_ENCRYPTED_OTA_USE_ECIES */
...
static esp_err_t esp_encrypted_img_export_rsa_pub_key(char *rsa_pem, size_t rsa_len, uint8_t **pub_key, size_t *pub_key_len)
{
    int ret = 0;
    if (rsa_pem == NULL) {
    ...
    *pub_key_len = max_pub_key_size - (c - *pub_key);
    if (*pub_key_len > max_pub_key_size) {
...
static esp_err_t esp_encrypted_img_export_ecies_pub_key(hmac_key_id_t hmac_key, uint8_t **pub_key, size_t *pub_key_len)
{
    ...
    mbedtls_ecp_point ecp_pub_key;
    mbedtls_ecp_group grp;

    mbedtls_mpi_init(&ecc_priv_key);
    mbedtls_ecp_group_init(&grp);
    mbedtls_ecp_point_init(&ecp_pub_key);
    ...
    size_t max_pubkey_len = UNCOMPRESSED_SECP256R1_SIZE; // Uncompressed point prefix + max bytes for SECP256R1
    *pub_key = calloc(1, max_pubkey_len);
    ...
    // Convert the mbedtls_mpi * to mbedtls_ecp_point *
    ret = mbedtls_ecp_mul(&grp, &ecp_pub_key, &ecc_priv_key, &grp.G, mbedtls_esp_random, NULL);
    if (ret != 0) {
        ESP_LOGE(TAG, "failed\n  ! mbedtls_ecp_mul returned -0x%04x\n", (unsigned int) - ret);
        err = ESP_FAIL;
        goto exit;
    }
    mbedtls_platform_zeroize(&ecc_priv_key, sizeof(mbedtls_mpi));
    mbedtls_mpi_free(&ecc_priv_key);

    ret = mbedtls_ecp_point_write_binary(&grp, &ecp_pub_key, MBEDTLS_ECP_PF_UNCOMPRESSED,
                                         &max_pubkey_len, *pub_key, max_pubkey_len);
    if (ret != 0) {
        ESP_LOGE(TAG, "Failed to write binary public key: -0x%04x", (unsigned int) - ret);
        err = ESP_FAIL;
        goto exit;
    }
    *pub_key_len = max_pubkey_len;
    mbedtls_ecp_point_free(&ecp_pub_key);
    mbedtls_ecp_group_free(&grp);
...
esp_err_t esp_encrypted_img_export_public_key(esp_decrypt_handle_t ctx, uint8_t **pub_key, size_t *pub_key_len)
{
    ...
    esp_err_t err = esp_encrypted_img_export_rsa_pub_key(handle->rsa_pem, handle->rsa_len, pub_key, pub_key_len);
    return err;
#elif defined(CONFIG_PRE_ENCRYPTED_OTA_USE_ECIES)
    ...
    esp_err_t err = esp_encrypted_img_export_ecies_pub_key(handle->hmac_key, pub_key, pub_key_len);
    return err;
#else
