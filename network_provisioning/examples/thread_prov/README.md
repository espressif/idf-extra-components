...
* `network_provisioning` : provides manager, data structures and protocomm endpoint handlers for Thread configuration
...
Make sure to note down the BLE device name (starting with `PROV_`) displayed in the serial monitor log (eg. PROV_F72E6B). This will depend on the MAC ID and will be unique for every device.
