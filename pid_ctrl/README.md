...
- **IQmath** — one API per Q-format (`_iq1` .. `_iq30`): `pid_*_iqN()` with `pid_ctrl_parameter_iqN_t` / `pid_ctrl_config_iqN_t` / `pid_ctrl_block_handle_iqN_t`, plus an unsuffixed `_iq` API that follows `GLOBAL_IQ`
...
> [!NOTE]
> IQmath `_iqN` scalars are all `int32_t`, so `pid_compute`'s error / result and the parameter fields must be converted with the matching `_IQN()` helper.
