...
- An IQmath API per concrete format is added, e.g. `pid_new_control_block_iq24()`
  with `pid_ctrl_parameter_iq24_t` / `pid_ctrl_config_iq24_t` /
  `pid_ctrl_block_handle_iq24_t`
- The unsuffixed `_iq` API is unchanged in name: `pid_ctrl_parameter_iq_t` /
  `pid_ctrl_config_iq_t` / `pid_ctrl_block_handle_iq_t` and `pid_*_iq()` keep
  working. They are inline wrappers around the `_iqN` backend named by
  `GLOBAL_IQ` in the including translation unit, and have their own types so
  they can be mixed with the concrete `_iqN` ones in the same translation unit
