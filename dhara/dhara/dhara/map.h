 ...
 * Controlled by CONFIG_DHARA_MAP_PATH_CACHE under ESP-IDF (dhara/Kconfig),
 * else override via -DDHARA_MAP_PATH_CACHE=0/1.
 *
 * Nothing else here pulls in sdkconfig.h, so fetch it ourselves or the
 * Kconfig guard below never sees CONFIG_ and always enables the cache.
 */
#if !defined(DHARA_MAP_PATH_CACHE) && __has_include("sdkconfig.h")
#include "sdkconfig.h"
#endif

#ifndef DHARA_MAP_PATH_CACHE
#if defined(CONFIG_IDF_TARGET) && !defined(CONFIG_DHARA_MAP_PATH_CACHE)
/* CONFIG_ bools are only defined when 'y'; absence here means 'n'. */
#define DHARA_MAP_PATH_CACHE  0
#else
