...
netif_op cmd_list[] = {
    ...
    {.name = "netif",     .operation = ifcfg_netif_op,  .arg_cnt = 4, .start_index = 1, .netif_flag = false,  .help = "ifconfig netif create/destroy <key id>/<iface>: Create or destroy a network interface with the specified key ID or interface name"},
    {.name = "eth",       .operation = ifcfg_eth_op,    .arg_cnt = 3, .start_index = 1, .netif_flag = false,  .help = "ifconfig eth: TBD"},
...
void print_iface_details(esp_netif_t *esp_netif)
{
    ...
    if (ESP_OK == esp_netif_dhcps_get_status(esp_netif, &status)) {
        ...
        if (ESP_NETIF_DHCP_STOPPED == status) {
            ...
            ESP_LOGI(TAG, "DHCP Client Status: %d", status);
        }
    }
...
esp_err_t get_netif_config(uint16_t id, esp_netif_config_t *eth_cfg_o)
{
    ...
    char *if_key;
    if (asprintf(&if_key, "IFC_EATH%d", id) == -1) {
        return ESP_FAIL;
    }
...
esp_err_t ifcfg_netif_op(netif_op *self, int argc, char *argv[], esp_netif_t *esp_netif)
{
    static esp_vlan_netifs netif_list;

    if (!strcmp(argv[self->start_index + 1], "create")) {
        esp_netif_config_t eth_cfg;
        ESP_ERROR_CHECK(get_netif_config(atoi(argv[self->start_index + 2]), &eth_cfg));
