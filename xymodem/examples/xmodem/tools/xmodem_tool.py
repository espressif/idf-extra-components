...
def cmd_send(args: argparse.Namespace) -> int:
    ...
    with open_serial(args.port, args.baud, args.timeout) as ser:
        print(f"Sending {len(payload)} bytes from {name} on {args.port} @ {args.baud} ({args.mode})", flush=True)
...
def cmd_recv(args: argparse.Namespace) -> int:
    ...
    with open_serial(args.port, args.baud, args.timeout) as ser:
        print(f"Receiving on {args.port} @ {args.baud} ({args.mode})", flush=True)
        modem = make_modem(ser, args.mode)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--retry", type=int, default=10, help="Protocol retry count (default: 10)")
