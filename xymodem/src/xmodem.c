 ...
 */
static esp_err_t purge_rx(const xymodem_transport_t *transport, uint32_t idle_timeout_ms, uint32_t overall_timeout_ms)
{
    uint8_t dummy[32];
...
static esp_err_t send_packet(const xymodem_transport_t *transport, const xymodem_send_config_t *config, const void *data, size_t len)
{
    for (uint32_t attempt = 0; attempt <= config->max_retry_cnt; attempt++) {
        ...
        while (true) {
            ...
            if (response == CAN) {
                recv_len = 0;
                ESP_RETURN_ON_ERROR(transport->recv(transport->ctx, &response, 1, &recv_len, config->can_interval_ms), TAG, "Recv failed");
                if (recv_len == 0) {
...
esp_err_t xmodem_send(const xymodem_transport_t *transport, const xymodem_send_config_t *config, const xmodem_data_source_t *source)
{
    ...
    ESP_RETURN_ON_FALSE(config->response_timeout_ms > 0, ESP_ERR_INVALID_ARG, TAG, "response timeout must > 0");
    ESP_RETURN_ON_FALSE(config->cancel_can_count >= 2, ESP_ERR_INVALID_ARG, TAG, "cancel CAN count must >= 2");
    ESP_RETURN_ON_FALSE(config->can_interval_ms > 0, ESP_ERR_INVALID_ARG, TAG, "CAN interval must > 0");
    ...
    for (uint32_t attempt = 0; attempt <= config->max_retry_cnt; attempt++) {
        uint8_t response;
        size_t recv_len = 0;
        ESP_RETURN_ON_ERROR(transport->recv(transport->ctx, &response, 1, &recv_len, config->response_timeout_ms), TAG, "Recv failed");
        ...
        if (response == NAK) {
            ...
            ESP_RETURN_ON_ERROR(purge_rx(transport, config->purge_idle_timeout_ms, config->response_timeout_ms), TAG, "Failed to purge RX buffer");
        }
    }
    ...
    for (uint8_t blk = 1; ; blk++) {
        ...
        size_t data_len = 0;
        ESP_GOTO_ON_ERROR(source->read(source->ctx, packet->data, request_len, &data_len), cleanup, TAG, "Read data failed");
        ESP_GOTO_ON_FALSE(data_len <= request_len, ESP_ERR_INVALID_SIZE, cleanup, TAG, "Read length exceeds request");
        if (data_len == 0) {
...
{
    size_t recv_len = 0;
    ESP_RETURN_ON_ERROR(transport->recv(transport->ctx, buffer, 1, &recv_len, config->packet_timeout_ms), TAG, "Recv failed");
    ...
    case CAN:
        recv_len = 0;
        ESP_RETURN_ON_ERROR(transport->recv(transport->ctx, buffer, 1, &recv_len, config->can_interval_ms), TAG, "Recv failed");
    ...
    ESP_RETURN_ON_ERROR(transport->recv(transport->ctx, buffer + 1, remaining_size, &recv_len, config->packet_timeout_ms), TAG, "Recv failed");
                             ...
                             void *buffer, xymodem_check_type_t check_type, uint8_t initial_request, uint8_t idle_request)
{
    ...
    for (uint32_t attempt = 0; attempt <= config->max_retry_cnt; attempt++) {
        ...
        case ESP_ERR_INVALID_CRC:
            packet_started = true;
            request = NAK;
            ESP_RETURN_ON_ERROR(purge_rx(transport, config->purge_idle_timeout_ms, config->packet_timeout_ms), TAG, "Failed to purge RX buffer");
            break;
        default:
            return ret;
        }
...
esp_err_t xmodem_recv(const xymodem_transport_t *transport, const xymodem_recv_config_t *config, const xmodem_data_sink_t *sink)
{
    ...
    ESP_RETURN_ON_FALSE(config->packet_timeout_ms > 0, ESP_ERR_INVALID_ARG, TAG, "packet timeout must > 0");
    ESP_RETURN_ON_FALSE(config->cancel_can_count >= 2, ESP_ERR_INVALID_ARG, TAG, "cancel CAN count must >= 2");
    ESP_RETURN_ON_FALSE(config->can_interval_ms > 0, ESP_ERR_INVALID_ARG, TAG, "CAN interval must > 0");
