 ...
 * - `ESP_ERR_NO_MEM`: No enough memory to create the RMT UART controller.
 */
 ...
 *
 * @note You can use the `rmt_uart_register_tx_event_callbacks()` function to register a callback function to handle the transmission done event.
 ...
 */
esp_err_t uart_emu_transmit(rmt_uart_device_handle_t uart_device, const uint8_t *data, size_t size);

/**
 * @brief Receive data over a UART emulator. This is a non-blocking function.
 *
 * @note You can use the `rmt_uart_register_rx_event_callbacks()` function to register a callback function to handle the received data.
 *
 * @param[in] uart_device Handle to the RMT UART controller, which was previously created using
 *                        `rmt_new_uart_device()`.
 ...
 */
esp_err_t uart_emu_receive(rmt_uart_device_handle_t uart_device, uint8_t *buf, size_t size);

/**
 * @brief Delete a RMT UART controller.
 *
 * @param[in] uart_device Handle to the RMT UART controller, which was previously created by `rmt_new_uart_device()`.
 *
 * @return
 ...
 *
 * @note After receiving the RMT UART symbols, you can use this function to decode the data.
 *
 * @param[in] uart_device Handle to the RMT UART controller, which was previously created by `rmt_new_uart_device()`.
 * @param[in] rmt_rx_evt_data Pointer to the RMT UART receive event data.
 ...
 *
 * @note This function allows the user to register callback functions to handle specific RMT UART reception completion event.
 *       The callbacks provide a mechanism to handle asynchronous events generated by the RMT UART controller.
 *
 * @param[in] uart_device Handle to the RMT UART controller, which was previously created by `rmt_new_uart_device()`.
 ...
 *
 * @note This function allows the user to register callback functions to handle specific RMT UART transmission completion event.
 *       The callbacks provide a mechanism to handle asynchronous events generated by the RMT UART controller.
 *
 * @param[in] uart_device Handle to the RMT UART controller, which was previously created by `rmt_new_uart_device()`.
