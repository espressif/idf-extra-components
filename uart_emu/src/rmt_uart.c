...
#define UART_RMT_DEFAULT_MEM_BLOCK_SYMBOLS   1024
#else
#define UART_RMT_DEFAULT_MEM_BLOCK_SYMBOLS   SOC_RMT_MEM_WORDS_PER_CHANNEL
#endif
...
// use higher priority to avoid timing issues
#define UART_RMT_DEFAULT_INTR_PRIORITY 3

#define ROUND_CLOSEST(dividend, divisor) (((dividend) + ((divisor) / 2)) / (divisor))

typedef struct rmt_uart_tx_context_t rmt_uart_tx_context_t;
typedef struct rmt_uart_rx_context_t rmt_uart_rx_context_t;
typedef struct rmt_uart_decode_context_t rmt_uart_decode_context_t;
typedef struct rmt_uart_device_t rmt_uart_device_t;

struct rmt_uart_tx_context_t {
    ...
    rmt_uart_tx_done_callback_t on_tx_trans_done;   // callback function for tx done event
    void *user_ctx;                                 // user context for tx done callback
};

struct rmt_uart_rx_context_t {
...
static bool rmt_uart_process_level(uint8_t level, uint32_t duration, int bit_ticks,
                                   const rmt_uart_device_t *uart_device,
                         ...
                         uint8_t *rx_buf, size_t rx_buf_size, bool continue_on_error)
{
    ...
    for (size_t i = 0; i < num_symbols; i++) {
        ...
        if (!rmt_uart_process_level(sym->level1, sym->duration1, RMT_BIT_RESOLUTION, uart_device, &decode_context, rx_buf, rx_buf_size)) {
            break;
        }
    }
...
IRAM_ATTR bool uart_rmt_rx_done_callback(rmt_channel_handle_t channel, const rmt_rx_done_event_data_t *edata, void *user_data)
{
...
IRAM_ATTR bool uart_rmt_tx_done_callback(rmt_channel_handle_t channel, const rmt_tx_done_event_data_t *edata, void *user_data)
{
    BaseType_t high_task_woken = pdFALSE;
    rmt_uart_device_t *uart_device = (rmt_uart_device_t *)user_data;
...
esp_err_t rmt_new_uart_device(const rmt_uart_config_t *uart_config, rmt_uart_device_handle_t *ret_device)
{
    ...
    ESP_RETURN_ON_FALSE(uart_config, ESP_ERR_INVALID_ARG, TAG, "Invalid argument");
    ...
    if (uart_config->rx_io_num != GPIO_NUM_NC) {
        ...
        rmt_rx_event_callbacks_t rx_cbs = {
            .on_recv_done = uart_rmt_rx_done_callback
        };
    ...
    if (uart_config->tx_io_num != GPIO_NUM_NC) {
        ...
        rmt_tx_channel_config_t uart_tx_channel_cfg = {
            ...
            .mem_block_symbols = UART_RMT_DEFAULT_MEM_BLOCK_SYMBOLS,
            .resolution_hz = resolution,
            .trans_queue_depth = uart_config->tx_trans_queue_depth,
            .intr_priority = UART_RMT_DEFAULT_INTR_PRIORITY,
        ...
        ESP_RETURN_ON_ERROR(rmt_new_uart_encoder(uart_config, &tx_context->tx_encoder), TAG, "new encoder failed");

        // register rmt tx done callback
        rmt_tx_event_callbacks_t tx_cbs = {
            .on_trans_done = uart_rmt_tx_done_callback
        };
    ...
    *ret_device = uart_device;
    ESP_LOGI(TAG, "new rmt uart at %p, baud=%d  rmt_resolution=%d", uart_device, uart_config->baud_rate, resolution);
    return ESP_OK;
...
esp_err_t rmt_delete_uart_device(rmt_uart_device_t *uart_device)
{
    ESP_RETURN_ON_FALSE(uart_device, ESP_ERR_INVALID_ARG, TAG, "Invalid argument");
