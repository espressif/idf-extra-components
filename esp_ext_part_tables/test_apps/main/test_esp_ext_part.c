...
static esp_err_t bdl_simulated_erase(esp_blockdev_handle_t handle, uint64_t start_addr, size_t erase_len)
{
    if (handle == NULL) {
        return ESP_ERR_INVALID_ARG;
    }
    ...
    memset(buffer + start_addr, handle->device_flags.default_val_after_erase, erase_len);
    return ESP_OK;
}
...
TEST_CASE("Test with BDL (simulated in RAM) - MBR related", "[esp_ext_part_table]")
{
    ...
    err = esp_ext_part_list_read(handle, &part_list, ESP_EXT_PART_LIST_SIGNATURE_MBR, (void *) &mbr_parse_args);
    TEST_ESP_OK(err);
    ...
    err = esp_ext_part_list_write(handle, &part_list, ESP_EXT_PART_LIST_SIGNATURE_MBR, (void *) &mbr_gen_args);
    TEST_ESP_OK(err);
