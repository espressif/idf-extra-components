...
static void ext_part_list_item_do_extra(esp_ext_part_list_item_t *item, mbr_partition_t *partition)
{
    ...
    case ESP_EXT_PART_TYPE_LITTLEFS:
        ...
        item->info.extra = (void*) esp_mbr_chs_arr_val_get(partition->chs_start); // Put LittleFS block size which was stored in `chs_start` to `extra` field
        break;
    default:
        break;
    }
