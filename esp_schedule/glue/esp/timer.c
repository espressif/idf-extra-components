...
void esp_schedule_timer_reset(esp_schedule_timer_handle_t timer_handle, uint32_t delay_seconds)
{
    ...
    uint64_t total_ticks = __ticks_from_seconds(delay_seconds);
    TickType_t period = __period_ticks(total_ticks);
    __timer_priv_data_t *timer_priv_data = __timer_priv_data_get((TimerHandle_t) timer_handle);
    if (timer_priv_data != NULL) {
        timer_priv_data->remaining_ticks = (total_ticks > period) ? (total_ticks - period) : 0;
    }
    /* This is reached both from the app task and, via the re-arm path, from the
     * timer daemon task inside a callback. A callback must not block on the
     * timer command queue, so use a zero block time in both cases. */
    xTimerChangePeriod((TimerHandle_t) timer_handle, period, 0);
}
