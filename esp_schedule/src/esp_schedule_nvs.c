...
ESP_SCHEDULE_RETURN_TYPE esp_schedule_nvs_add(esp_schedule_t *schedule)
{
    ...
    if (nvs_priv_data_callbacks.on_save != NULL && private_data_size > 0) {
        ...
        if (data == NULL || data_size != private_data_size) {
            ESP_SCHEDULE_LOGE(TAG, "priv data size mismatch between on_save calls (%u vs %u); aborting save",
    ...
    if (editing_schedule == false) {
        ...
        if (schedule_count == UINT8_MAX) {
            ESP_SCHEDULE_LOGE(TAG, "Schedule count at maximum (%u); not adding %s", UINT8_MAX, schedule->name);
            ESP_SCHEDULE_FREE(blob_buffer);
            esp_schedule_nvs_close(nvs_handle);
            return ESP_SCHEDULE_RET_NO_MEM;
        }
    ...
    if (editing_schedule == false) {
        err = esp_schedule_nvs_set_u8(nvs_handle, ESP_SCHEDULE_COUNT_KEY, (uint8_t)(schedule_count + 1));
        if (err != ESP_SCHEDULE_NVS_OK) {
            ...
            esp_schedule_nvs_erase_key(nvs_handle, schedule->name);
 ...
 * raw esp_schedule_t via esp_schedule_nvs_set_blob(..., sizeof(esp_schedule_t)). The first
...
static esp_schedule_handle_t esp_schedule_migrate_from_v1(const char *nvs_key, const uint8_t *blob, size_t buf_size)
{
    if (buf_size != sizeof(esp_schedule_legacy_v1_t)) {
        ESP_SCHEDULE_LOGE(TAG, "Schedule %s blob is neither current format nor a known legacy layout (%u bytes); rejecting",
    ...
    if (v1->trigger.type == ESP_SCHEDULE_TYPE_INVALID) {
        ESP_SCHEDULE_LOGE(TAG, "Schedule %s legacy blob has invalid trigger type; rejecting", nvs_key);
        return NULL;
    }

    esp_schedule_t *schedule = (esp_schedule_t *)ESP_SCHEDULE_CALLOC(1, sizeof(esp_schedule_t));
    if (schedule == NULL) {
        ESP_SCHEDULE_LOGE(TAG, "Could not allocate schedule while migrating %s", nvs_key);
        return NULL;
    }
    ...
    ESP_SCHEDULE_LOGW(TAG, "Migrated schedule %s from legacy (pre-2.0) NVS format", nvs_key);
...
static esp_schedule_handle_t esp_schedule_nvs_get(const char *nvs_key)
{
    ...
    if (buf_size < schedule_size) {
        ESP_SCHEDULE_LOGE(TAG, "Schedule %s blob too small (%u < %u); rejecting", nvs_key, (unsigned)buf_size, (unsigned)schedule_size);
        ESP_SCHEDULE_FREE(blob_buffer);
        return NULL;
    }
    esp_schedule_persistent_t *persistent = (esp_schedule_persistent_t *)(blob_buffer);
    if (persistent->version != ESP_SCHEDULE_NVS_FORMAT_VERSION) {
        ...
        ESP_SCHEDULE_LOGW(TAG, "Schedule %s blob is not current format (version byte %u); attempting legacy migration", nvs_key, persistent->version);
    ...
    if (persistent->struct_size != (uint16_t) schedule_size) {
        ESP_SCHEDULE_LOGE(TAG, "Schedule %s struct size %u != %u; rejecting", nvs_key, persistent->struct_size, (unsigned)schedule_size);
        ESP_SCHEDULE_FREE(blob_buffer);
        return NULL;
    }
    ...
    *schedule_count = esp_schedule_nvs_get_count();
    ...
    while (err == ESP_SCHEDULE_NVS_OK) {
        ...
        if (handle_count >= *schedule_count) {
            ESP_SCHEDULE_LOGW(TAG, "More schedule blobs than count (%u); ignoring extra key %s", *schedule_count, next_key);
...
ESP_SCHEDULE_RETURN_TYPE esp_schedule_nvs_init(char *nvs_partition, esp_schedule_priv_data_callbacks_t *priv_data_callbacks)
{
    ...
    if (nvs_enabled) {
        ESP_SCHEDULE_LOGI(TAG, "NVS already enabled; callbacks updated");
        return ESP_SCHEDULE_RET_OK;
    }
    if (nvs_partition) {
