...
TEST_CASE("esp_cli() loop calls all callbacks and exit on call to esp_cli_stop", "[esp_cli][host_test]")
{
...
TEST_CASE("esp_cli() exits when esp_cli_stop() called from the task running esp_cli()", "[esp_cli][host_test]")
{
    SemaphoreHandle_t start_sem, done_sem;
    esp_linenoise_handle_t linenoise_hdl;
    esp_cli_handle_t cli_hdl;
