...
// Context for the command (must be constant for static registration)
...
void app_main(void)
{
    ...
    int ret = -1;
    esp_err_t err = esp_cli_commands_execute("help", &ret, NULL, NULL);
    if (err == ESP_OK) {
    ...
    ret = -1;
    err = esp_cli_commands_execute("hello", &ret, NULL, NULL);
    if (err == ESP_OK) {
