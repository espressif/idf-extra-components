 ...
 */
typedef int (*esp_command_func_t)(void *context, int argc, char **argv);
...
typedef struct esp_commands_config {
    ...
    bool hint_bold;              /*!< Set to 1 to print hint text in bold */
...
/**
 * @brief Update the configuration of the component with the provided one
 *
 * @param config configuration data to initialize the component
 * @return esp_err_t ESP_OK Initialization was successful
 *                   ESP_ERR_INVALID_ARG Invalid pointer to config
 */
 ...
 *
 * @param cmd_set set of commands allowed to be ran. If the command from the cmd_line is
 * not in the set, it will not be executed and will yield a ESP_ERR_NOT_FOUND return value.
 * If cmd_set is set to NULL, all the registered functions will be used as the command set.
 ...
 *        passed as parameter
 *      - ESP_ERR_NO_MEM, if the internal variables could not get allocated for lack of
 *        memory
 */
 ...
 *
 * @param cmd_set set of commands allowed to be completed. If the command from the cmd_line is
 * not in the set, it will not be transferred to completion_cb. If cmd_set is set to NULL, all
 * the registered functions will be used as the command set.
 ...
 *
 * @param cmd_set set of commands allowed to be completed. If the command from the cmd_line is
 * not in the set, NULL will be returned by the function. If cmd_set is set to NULL, all the
 * registered functions will be used as the command set.
 ...
 * @param cmd_set_size the number of items in cmd_set
 * @param get_field a function pointer to the function returning the correct field in esp_command_t
 * with which to compare the strings in cmd_set
...
/**
 * @brief Wrap the call to esp_commands_create_cmd_set to avoid having to manually
 * pass cmd_set_size
 */
