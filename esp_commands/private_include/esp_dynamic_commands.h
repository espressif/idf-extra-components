...
typedef struct esp_command_set {
    esp_command_t **cmd_ptr_set; /*!< Array of pointers to commands. */
 ...
 *
 * Each dynamic command is stored as an `esp_command_t` plus
 * linked list metadata for insertion/removal.
 */
typedef struct esp_command_internal {
    esp_command_t cmd; /*!< Command instance. */
    SLIST_ENTRY(esp_command_internal) next_item; /*!< Linked list entry metadata. */
} esp_command_internal_t;

/**
 * @brief Linked list head type for dynamic command storage.
 */
typedef SLIST_HEAD(esp_command_internal_ll, esp_command_internal) esp_command_internal_ll_t;

/**
 * @brief Iterate over a set of commands, either from a static set or dynamic list.
 *
 * This macro supports iterating over:
 * - A provided `esp_command_set_t` (static set), OR
 * - The global dynamic command list if `cmd_set` is `NULL`.
 *
 * @param cmd_set Pointer to a command set (`esp_command_set_t`) or `NULL` for dynamic commands.
 * @param item_cmd Iterator variable of type `esp_command_t *` that will point to each command.
 *
 * @note Internally, the macro uses `_node` and `_i` as hidden variables.
 */
#define FOR_EACH_DYNAMIC_COMMAND(cmd_set, item_cmd)                             \
    __attribute__((unused)) esp_command_internal_t *_node =                     \
            ((cmd_set) == NULL ? SLIST_FIRST(esp_dynamic_commands_get_list())  \
                                : NULL);                                        \
