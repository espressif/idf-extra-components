...
# Install dependencies at module import time to ensure they're available for tests
    ...
    if result.returncode != 0:
        ...
        if result.returncode != 0:
            ...
            try:
                ...
                # Last attempt: try with verbose output for debugging
                print(f'Warning: Failed to install requirements. Attempting verbose install...')
...
# Install dependencies when module is imported
...
def build_project(project_dir):
    ...
    # Get IDF_PATH from environment (set by pytest-embedded-idf)
    idf_path = os.environ.get('IDF_PATH')
    if not idf_path:
        raise Exception('IDF_PATH environment variable is not set. ESP-IDF environment is not available.')
    
    # Construct path to idf.py: $IDF_PATH/tools/idf.py
    idf_py = os.path.join(idf_path, 'tools', 'idf.py')
    if not os.path.exists(idf_py):
        raise Exception(f'idf.py not found at {idf_py}. ESP-IDF may not be properly installed.')
    
    # Run idf.py using Python (it's a Python script)
...
def generate_patch(base_binary, new_binary, patch_output, chip='esp32'):
    ...
    # Dependencies are already installed at module import time
