...
{
    TEST_ASSERT_NOT_NULL(arg_rem(NULL, "comment"));
    TEST_ASSERT_NOT_NULL(arg_lit0("v", "verbose", "Enable verbose"));
    TEST_ASSERT_NOT_NULL(arg_lit1("f", "force", "Force operation"));

    TEST_ASSERT_NOT_NULL(arg_int0("i", "int", "<n>", "Optional int"));
    TEST_ASSERT_NOT_NULL(arg_int1("i", "int", "<n>", "Required int"));
    TEST_ASSERT_NOT_NULL(arg_intn("i", "int", "<n>", 0, 3, "Multiple ints"));

    TEST_ASSERT_NOT_NULL(arg_str0("s", "str", "<str>", "Optional string"));
    TEST_ASSERT_NOT_NULL(arg_str1("s", "str", "<str>", "Required string"));
    TEST_ASSERT_NOT_NULL(arg_strn("s", "str", "<str>", 1, 3, "Multi string"));

    TEST_ASSERT_NOT_NULL(arg_file0("f", "file", "<file>", "Optional file"));
    TEST_ASSERT_NOT_NULL(arg_file1("f", "file", "<file>", "Required file"));
    TEST_ASSERT_NOT_NULL(arg_filen("f", "file", "<file>", 1, 3, "Multi file"));

    TEST_ASSERT_NOT_NULL(arg_dbl0("d", "double", "<d>", "Optional double"));
    TEST_ASSERT_NOT_NULL(arg_dbl1("d", "double", "<d>", "Required double"));
    TEST_ASSERT_NOT_NULL(arg_dbln("d", "double", "<d>", 1, 2, "Multi double"));

    TEST_ASSERT_NOT_NULL(arg_date0("t", "time", "<date>", "%Y-%m-%d", "Optional date"));
    TEST_ASSERT_NOT_NULL(arg_date1("t", "time", "<date>", "%Y-%m-%d", "Required date"));

    TEST_ASSERT_NOT_NULL(arg_rex0("r", "regex", "<expr>", "^[a-z]+$", 0, "Regex"));
    TEST_ASSERT_NOT_NULL(arg_rex1("r", "regex", "<expr>", "^[a-z]+$", 0, "Regex"));

    TEST_ASSERT_NOT_NULL(arg_end(5));
}
...
TEST_CASE("arg_print_syntax, glossary, and GNU glossary", "[argtable3]")
{
    ...
    TEST_ASSERT_NOT_NULL(strstr(buf, "--verbose"));
    ...
    fflush(f);
    TEST_ASSERT_NOT_NULL(strstr(buf, "verbose"));
    TEST_ASSERT_NOT_NULL(strstr(buf, "Name is required"));
    fclose(f);

    memset(buf, 0, sizeof(buf));
    f = fmemopen(buf, sizeof(buf), "w");
    TEST_ASSERT_NOT_NULL(f);
    arg_print_glossary_gnu(f, argtable);
    fflush(f);
    TEST_ASSERT_NOT_NULL(strstr(buf, "--verbose"));
...
TEST_CASE("arg_print_errors prints expected message", "[argtable3]")
{
    ...
    TEST_ASSERT_NOT_NULL(strstr(buf, "option -n is required"));
...
TEST_CASE("returns errors for invalid input", "[argtable3]")
{
    ...
    arg_print_errors(f, end, argv[0]);
    fclose(f);
    TEST_ASSERT_NOT_NULL(strstr(buf, "invalid integer"));
